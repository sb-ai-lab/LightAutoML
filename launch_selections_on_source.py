...
source = "kaggle"
names_versions = [
    [i["name"], i["version"]] for i in Dataset.list_datasets(dataset_project="Datasets_with_metadata", tags=[source])
]


for dataset_name, dataset_version in tqdm(names_versions):
    task = Task.create(
        project_name="selection_{0}_1".format(source),
        ...
        argparse_args=[("dataset", dataset_name), ("source", source), ("dataset_version", dataset_version)],
    )
