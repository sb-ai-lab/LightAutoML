...
if __name__ == "__main__":
    ...
    assert res.data.shape == res_gpu.data.shape == (res_mgpu.data.shape[0].compute(),
                                                    res_mgpu.data.shape[1])
    pd.testing.assert_frame_equal(res_gpu.data.to_pandas(), res.data)
    #pd.testing.assert_frame_equal(res_mgpu.data.compute().to_pandas(), res.data)
    pd.testing.assert_frame_equal(res_gpu.seq_data['data2'].data.to_pandas(), res.seq_data['data2'].data)
    #pd.testing.assert_frame_equal(res_mgpu.seq_data['data2'].data.compute().to_pandas(), res.seq_data['data2'].data)
    #to_sequence() asserts
    ...
    #maybe asserts about isinstance cudf.DataFrame and so on?
