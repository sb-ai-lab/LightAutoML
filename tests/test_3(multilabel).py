...
def generate_data(n, n_num, n_cat, n_date, n_str, max_n_cat):
    ...
    data['TARGET0'] = pd.Series(np.random.randint(0, 2, n)).astype('i')
    data['TARGET1'] = pd.Series(np.random.randint(0, 2, n)).astype('i')
    data['TARGET2'] = pd.Series(np.random.randint(0, 2, n)).astype('i')
...
if __name__ == "__main__":
    ...
    #task_cpu = Task("multilabel")
    #task_gpu = Task("multilabel", device="gpu")
    task_mgpu = Task("multilabel", device="mgpu")
    ...
    print("mgpu time: ", perf_counter() - st)
