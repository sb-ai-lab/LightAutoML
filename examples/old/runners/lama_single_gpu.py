...
if __name__ == '__main__':
    ...
    t = time()
    oof_predictions = automl.fit_predict(data[folds!=args.fold].reset_index().drop(['index'],axis=1), roles=roles, verbose=4)
    results['train_time'] = time() - t
    
    # VALID
    
    t = time()
    test_pred = automl.predict(data[folds==args.fold].reset_index().drop(['index'],axis=1)).data
