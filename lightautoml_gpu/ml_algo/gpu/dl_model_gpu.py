...
from lightautoml_gpu.ml_algo.base import TabularDataset
from lightautoml_gpu.ml_algo.gpu.base_gpu import TabularMLAlgoGPU
...
from lightautoml_gpu.text.nn_model import CatEmbedder
from lightautoml_gpu.text.nn_model import ContEmbedder
from lightautoml_gpu.text.nn_model import TextBert
from lightautoml_gpu.text.nn_model import TorchUniversalModel
from lightautoml_gpu.text.nn_model import UniversalDataset
from lightautoml_gpu.text.gpu.trainer_gpu import TrainerGPU
from lightautoml_gpu.text.utils import inv_sigmoid
from lightautoml_gpu.text.utils import inv_softmax
from lightautoml_gpu.text.utils import is_shuffle
from lightautoml_gpu.text.utils import parse_devices
from lightautoml_gpu.text.utils import seed_everything
...
class TorchModelGPU(TabularMLAlgoGPU):
    ...
    def get_mean_target(target, task_name):
        bias = (
            ...
            else np.unique(target.values.get(), return_counts=True)[1]
        )
        bias = (
            inv_sigmoid(bias)
    ...
    def get_dataloaders_from_dicts(self, data_dict):
        ...
        for stage, value in data_dict.items():
            ...
            data["text"] = value.data[self.params["text_features"]].to_pandas().values

            datasets[stage] = self.train_params["dataset"](
                data=data,
                y=torch.as_tensor(value.target.values, device='cuda:0') if stage != "test" else torch.ones(len(value.data), dtype=int, device='cuda:0'),
                w=torch.as_tensor(value.weights.values, device='cuda:0') if value.weights is not None else torch.ones(len(value.data), dtype=int, device='cuda:0'),
