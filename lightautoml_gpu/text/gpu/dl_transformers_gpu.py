...
from lightautoml_gpu.text.dp_utils import CustomDataParallel
from lightautoml_gpu.text.sentence_pooling import SequenceAvgPooler
from lightautoml_gpu.text.sentence_pooling import SequenceClsPooler
from lightautoml_gpu.text.sentence_pooling import SequenceIndentityPooler
from lightautoml_gpu.text.sentence_pooling import SequenceMaxPooler
from lightautoml_gpu.text.sentence_pooling import SequenceSumPooler
from lightautoml_gpu.text.utils import _dtypes_mapping
from lightautoml_gpu.text.utils import parse_devices
from lightautoml_gpu.text.utils import seed_everything
from lightautoml_gpu.text.utils import single_text_hash
        ...
        for sample in loader:
            ...
            if self.embedding_model is not None:
                ...
                if "attention_mask" in data:
                    ...
                    length = (torch.ones(len(embed), device=self.device) * self.dataset_params["max_length"]).long()
                data = {"text": embed, "length": length}
            embed = cp.asarray(self.model(data).detach())
            result.append(embed.astype(cp.float32))
...
class BOREPGPU(nn.Module):
    ...
    def forward(self, inp: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        x_length = (torch.arange(out.shape[1], device=out.device)[None, :] < inp["length"][:, None])[:, :, None]
...
class BertEmbedderGPU(nn.Module):
    ...
    def forward(self, inp: Dict[str, torch.Tensor]) -> torch.Tensor:
        self.transformer = self.transformer.to(inp["input_ids"].device)
