...
class TrainerGPU:
    ...
    def train(self, dataloaders: Dict[str, DataLoader]) -> List[float]:
        ...
        for sample in loader:
            data = {
                i: (sample[i].long().to(self.device) if i in ["input_ids", "attention_mask", "token_type_ids"] else
                    sample[i].long() if _dtypes_mapping[i] == "long" else
                    sample[i].float().to(self.device) if i == "text" else
                    sample[i].float())
